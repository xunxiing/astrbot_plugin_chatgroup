...
良好的错误处理机制，不要让插件因为一个错误而崩溃。
在进行任何一个提交之前，请使用 ruff 工具格式化您的代码。
开发指南
CAUTION
...
关于 self.html_render() 和 self.text_to_image()：
• 外部服务依赖：明确指出这些内置的渲染函数（尤其是在默认配置下）可能依赖外部网络服务（例如我们遇到的 https://t2i.soulter.top）。开发者应该意识到，如果这些外部服务不稳定或出错（如返回 HTTP 500），插件的渲染功能会失败，并可能抛出像 aiohttp.client_exceptions.ContentTypeError 这样的错误。
• 参数传递方式：强调 self.html_render(template_string, data_dict)的参数是位置参数，而不是关键字参数。文档中的示例是正确的，但开发者可能会习惯性使用关键字参数而导致 TypeError。
• 本地渲染选项：如果 AstrBot 支持将这些渲染功能配置为使用本地渲染引擎（例如 Playwright、Puppeteer 或其他基于浏览器的引擎），文档中应详细说明如何配置，以及本地渲染可能需要的系统依赖（如特定浏览器、相关库）。这能提供一个更稳定的替代方案，避免依赖外部服务。
2. 插件日志记录 (Logging)：
• AstrBot Logger 与标准 Logging：解释 from astrbot.api import logger as astrbot_logger 的推荐用法，以及它与 Python 标准 logging 模块的关系。如果 astrbot_logger 是推荐的主要方式，应说明其配置和预期行为。
• 确保日志可见性：提供明确的指导，帮助开发者确保他们插件中的日志（尤其是 DEBUG 级别和通过 exc_info=True 记录的完整错误堆栈）能够显示在 AstrBot 的主控制台或主日志文件中。这是我们调试过程中遇到的一个主要障碍。
• exc_info=True 的重要性：在 try...except 块中捕获异常时，强烈建议在调用 logger.error() 时使用 exc_info=True，以便记录完整的错误堆栈信息，这对于定位问题至关重要。
• 开发阶段的调试技巧：如果通过标准日志系统难以获取详细错误，可以建议开发者在开发阶段临时使用 print(traceback.format_exc()) 或将日志直接输出到特定文件，以强制获取调试信息。
3. 命令参数解析 (@filter.command)：
• 捕获剩余参数：详细说明当命令需要接收命令名之后的所有文本（包括空格）作为单个参数时（例如 /command <这里是包含空格的完整文本>），推荐的函数签名是什么。我们的经验表明，使用单个类型为 str 的参数（如 async def my_command(self, event: AstrMessageEvent, full_input: str):）是可行的。
• 多参数与空格：阐明默认情况下，如果函数签名包含多个参数，命令解析器是如何根据空格来分割用户输入的。
4. 处理外部库依赖（例如本地图像处理）：
• 明确依赖：如果插件引入了如 matplotlib, sympy 等需要额外安装的库，应在插件的 requirements.txt 文件中清晰列出。
• 异步环境中的阻塞操作：当使用像 matplotlib 这样的同步/阻塞库进行 I/O 密集型或 CPU 密集型操作（如本地图片渲染）时，应指导开发者使用 asyncio.get_event_loop().run_in_executor(None, blocking_function, ...) 来避免阻塞 AstrBot 的异步事件循环。
• 库的特定行为：提醒开发者注意某些库的特定行为。例如，matplotlib 的 mathtext 对复杂 LaTeX 的支持有限；sympy.preview() 在某些情况下可能会尝试调用系统级的 LaTeX 程序，除非明确配置为使用 matplotlib 后端。
5. 输入数据清理：
• HTML 实体解码：提示开发者，从聊天平台或 LLM 获取的文本输入可能包含 HTML 实体（例如 &amp; 而不是 &）。在处理这类文本（尤其是用于 LaTeX 等对特殊字符敏感的场景）之前，应使用 html.unescape() 进行解码。
• 字符串清理：在将字符串传递给渲染函数或外部库之前，使用 .strip() 清除首尾多余的空格。
6. 临时文件管理：
• 安全路径：建议插件在需要创建临时文件（如渲染的图片）时，在其自身的插件目录内创建一个子目录（例如 temp_images）来存放。使用 os.path.dirname(__file__) 可以安全地获取当前插件的路径。
• 唯一文件名：使用 uuid.uuid4().hex 等方式为临时文件生成唯一文件名，以避免冲突。
• 清理机制：考虑临时文件的清理策略（例如，发送后立即删除，或定期清理）。
7. 调试“静默失败”的技巧：
• 当插件的某部分功能（如发送消息或图片）似乎没有按预期工作，且主日志中没有明显错误时，可以建议一些调试方法：
• 在代码的关键步骤中 yield event.plain_result("调试消息：步骤 X 完成")，通过聊天反馈来追踪执行流程。
• 增加更细致的日志级别（DEBUG），并在每个逻辑分支和重要变量状态变化时都添加日志输出。